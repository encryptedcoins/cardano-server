...
import           Cardano.Server.Config          (Config (..), ServerEndpoint (..), schemeFromProtocol)
...
import           Servant.Client                 (BaseUrl (BaseUrl), ClientEnv (..), defaultMakeClientRequest)
...
createServantClientEnv Config{..} = do
    ...
    pure $ ClientEnv
        manager
        (BaseUrl (schemeFromProtocol cHyperTextProtocol) (T.unpack cHost) cPort "")
        Nothing
